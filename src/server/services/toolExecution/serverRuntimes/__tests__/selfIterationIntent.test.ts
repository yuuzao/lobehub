...
describe('selfIterationIntentRuntime', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('calls the enqueue boundary and returns accepted JSON content', async () => {
    const runtime = selfIterationIntentRuntime.factory({ toolManifestMap: {} });

    const result = await runtime.declareSelfIterationIntent(createInput(), {
      agentId: 'agent-1',
    ...
    expect(JSON.parse(result.content)).toEqual({
      accepted: true,
      reason: null,
      sourceId: 'self-iteration-intent:user-1:agent-1:operation:operation-1:tool-call-1',
      strength: 'strong',
    });
  ...
  it('returns success true with accepted false for rejected service results', async () => {
    ...
    const result = await runtime.declareSelfIterationIntent(
  ...
  it('preserves declaration rate-limit state across runtime factory calls', async () => {
    for (const toolCallId of ['tool-call-1', 'tool-call-2', 'tool-call-3']) {
      ...
      await expect(
        runtime.declareSelfIterationIntent(createInput(), {
          agentId: 'agent-1',
          toolCallId,
          toolManifestMap: {},
